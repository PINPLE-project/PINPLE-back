...
async function insertPin(connection, insertPinParams) {
  const insertPinQuery = `
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?);
      `;
  const insertPinRows = await connection.query(insertPinQuery, insertPinParams);
...
async function deleteMyPin(connection, userId, pinId) {
  const deleteMyPinQuery = `
                  ...
                  `;
  const deleteMyPinRows = await connection.query(
    deleteMyPinQuery,
    userId,
    pinId
  );
...
// 유저의 핀 알림 설정 여부 조회
async function selectPinAlarmStatus(connection, userId) {
  const selectPinAlarmStatusQuery = `
                                SELECT pinAlarm
                                FROM User
                                WHERE userId = ${userId};
                                `;
  const selectPinAlarmStatusRows = await connection.query(
    selectPinAlarmStatusQuery,
    userId
  );
  return selectPinAlarmStatusRows;
}

// 핀 알림을 위한 핀 작성자의 정보 조회
async function selectUserInfoForPinNoti(connection, userId) {
  const selectUserInfoForPinNotiQuery = `
                                    SELECT nickname, deviceToken
                                    FROM User
                                    WHERE userId = ${userId};
                                    `;
  const selectUserInfoForPinNotiRows = await connection.query(
    selectUserInfoForPinNotiQuery,
    userId
  );
  return selectUserInfoForPinNotiRows;
}
...
module.exports = {
  ...
  insertPinLikeUser,
  deletePinLikeUser,
  selectPinAlarmStatus,
  selectUserInfoForPinNoti,
};

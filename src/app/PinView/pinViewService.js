...
//핀지도 생성
exports.createPin = async function (
  userId,
  longitude,
  latitude,
  address,
  ...
  try {
    ...
    const nearByPinIdResult = await pinViewDao.insertNearByPin(
      connection,
      pinId,
      pinCongest
    );
...
exports.postPinLike = async function (pinId, userId) {
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    const likeCountResult = await pinViewDao.updatePinLikeCount1(
      connection,
      pinId
    );
    const likeUserResult = await pinViewDao.insertPinLikeUser(
      connection,
      pinId,
      userId
    );
...
exports.deletePinLike = async function (pinId, userId) {
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    const likeCountResult = await pinViewDao.updatePinLikeCount2(
      connection,
      pinId
    );
    const likeUserResult = await pinViewDao.deletePinLikeUser(
      connection,
      pinId,
      userId
    );
...
exports.deleteMyPin = async function (userId, pinId) {
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    const myPinIdResult = await pinViewDao.deleteMyPin(
      connection,
      userId,
      pinId
    );
